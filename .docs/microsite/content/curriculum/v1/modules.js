/* Smart CI - 30-Minute Delivery module content.
   Generated from the CI for CI (.docs/knowledge-base/modules) knowledge base. */
window.MODULES = {
"intro": {
  id: "intro",
  ...
  recap: [
    ...
    "CI is not a project; it is an operating model. Success is the customer running the loop without the practitioner",
  ...
  questions: [
    ...
    {
      prompt: "What is the practitioner's exit criterion for a successful CI engagement?",
      options: ["The customer has completed three PDCA cycles.", "All recommendations from the initial assessment have been implemented.", "The customer is running the loop without the practitioner.", "The customer has achieved the largest possible improvement."],
      correctIndex: 2,
      explanation: "Success in CI means <strong>the customer is running the cadence independently</strong>. The practitioner coaches the loop; success is when the customer owns it and the practitioner can exit without regression."
    },
    {
  ...
  group: "Foundations",
  duration: "30 min",
  executiveSummary: "Lean and Six Sigma certifications use a martial-arts belt metaphor—White, Yellow, Green, Black, Master Black—to signal the depth of continuous-improvement capability from knowing the vocabulary to designing org-wide CI strategy. For practitioners the belt ladder is useful as a personal skilling roadmap, a credential customers recognize (especially in regulated industries), and a coaching framework for customer teams. The key principle: treat each belt as the formalization of capability you should already be demonstrating in real engagements—credentials trail the work, not the other way around.",
  whatYouGain: [
  ...
  questions: [
    {
      ...
      options: ["Years of experience as a practitioner.", "Depth of continuous-improvement capability, from knowing the vocabulary to leading org-wide CI strategy.", "How many customer engagements they have closed in the last quarter.", "Their internal performance rating."],
      correctIndex: 1,
    ...
    {
      prompt: "How should a practitioner decide which belt to pursue next?",
      ...
      correctIndex: 2,
      explanation: "The guidance is to <strong>pick the belt that formalizes the next gap</strong> — not the highest available. A practitioner who has never run a real Kaizen should pursue Yellow, not Black. Credentials should trail the work, not the other way around."
    },
    {
    ...
    },
    {
      ...
      correctIndex: 1,
      explanation: "A practitioner with five belts and no validated engagement outcomes is <strong>less effective</strong> than one with a Green Belt and a track record of measured customer wins. The credential is corroborating evidence, not a substitute for the work."
    },
    {
  ...
  group: "Foundations",
  duration: "30 min",
  executiveSummary: "Continuous Improvement at Microsoft is the connective tissue between the customer-facing practitioner practice and the engineering groups that build Azure. It shows up as the CI Community of Practice, the WAF and Advisor feedback loops, the practitioner playbook repositories, and the cross-team rituals that compound learnings across the org. For a practitioner it is both a resource (skilling, playbooks, peers) and a responsibility (contribute the patterns you validate, escalate systemic issues to product groups). Done well, CI@MS turns every customer engagement into both an outcome for that customer and a reusable asset for every practitioner that follows.",
  whatYouGain: [
    "Understand how CI@MS aggregates learnings across thousands of practitioners and feeds them back to Azure product groups",
    "Learn to consume existing playbooks (WAF, practitioner Playbook Library, Azure Advisor) instead of re-inventing engagement patterns",
  ...
  explanation: "<p>CI@MS exists to solve a scale problem: a single practitioner owns a handful of accounts, but Microsoft has thousands of practitioners and tens of thousands of customers. Without a CI practice that aggregates learnings, every practitioner re-discovers the same patterns, every customer relives the same incidents, and product groups do not hear the signal clearly enough to fix root causes. CI@MS is the feedback loop that turns field observations into product improvements.</p><p>What distinguishes CI@MS structures is that they are <strong>measured, versioned, owned, and reusable</strong>. Every published pattern carries the evidence that it worked. Playbooks evolve cycle over cycle; old versions are deprecated, not deleted. Each playbook has a named maintainer and a refresh cadence. Patterns are written for the next practitioner to apply, not as anecdotes. The major surfaces include the CI Community of Practice, the WAF assessment framework, Azure Advisor and Defender for Cloud as per-account baselines, versioned practitioner / CSE / FastTrack playbooks, MS Learn curriculum, and structured field-to-PG escalation channels (ACR blockers, ICM signature analysis).</p><p>The rhythm of community participation is simple: consume existing playbooks before designing a new engagement; run PDCA on real customer work; document measured outcomes; bring patterns to the monthly CoP call; let peers review and critique; merge validated patterns into shared libraries. Practitioners both consume and contribute. Skipping either side breaks the loop. When the same root cause appears across 3+ accounts, that is a PG conversation, not a practitioner workaround. Use the ACR blocker or ICM signature channels to escalate systemic signals so engineering can prioritize fixes.</p>",
  ...
  recap: [
    "CI@MS is the connective tissue between field learnings and product-group priorities—it turns one practitioner's observation into org-wide improvement",
    "Every practitioner consumes existing playbooks (WAF, practitioner Playbook Library, Advisor guidance) before starting a new engagement",
    "Contribute validated patterns back to the CoP; let peers review; merge into shared libraries; watch them compound across thousands of practitioners",
    "When the same root cause appears across 3+ accounts, escalate as a field signal (ACR blocker or ICM signature) to product groups, not as a practitioner workaround",
  ...
  questions: [
    {
      ...
      options: ["To replace customer-specific engagement plans with a single corporate template.", "To aggregate learnings across practitioners and provide a data-driven feedback loop from the field to product groups.", "To certify practitioners in Azure services.", "To provide a centralized time-tracking system for the practitioner practice."],
      correctIndex: 1,
      explanation: "CI@MS is the connective tissue between the field and engineering. It <strong>aggregates patterns</strong> so PG can prioritize fixes, codifies validated engagement patterns into shared playbooks, and turns one practitioner's observation into a product-level improvement."
    },
    {
      prompt: "What distinguishes a CI@MS playbook from a generic &ldquo;internal Wiki of tips&rdquo;?",
      options: ["Playbooks are only accessible to senior practitioners.", "Playbooks are auto-generated from Azure telemetry.", "Playbooks are measured, versioned, owned by a named maintainer with a refresh cadence, and written to be reusable.", "Playbooks are written exclusively by product groups."],
      correctIndex: 2,
      explanation: "Each published pattern carries the <strong>evidence that it worked</strong>, evolves cycle over cycle, has a named owner, and is written for the next practitioner to apply — not as an anecdote."
    },
    {
      prompt: "A practitioner spots the same product limitation showing up across three or more of their accounts. What does CI@MS recommend they do?",
      ...
      correctIndex: 2,
      explanation: "When the same root cause appears across multiple accounts, that's a <strong>PG conversation, not a practitioner workaround</strong>. The field-to-PG signal channels (ACR blocker tracker, ICM signature analysis) exist to turn aggregated evidence into product-level CI."
    },
    {
    ...
    },
    {
      ...
      options: ["A practitioner who escalates every issue directly to PG without trying to resolve it first.", "A practitioner who only consumes playbooks but never contributes back.", "A practitioner who validates many patterns across their accounts but never publishes them — forcing every other practitioner to re-discover the same lessons.", "A practitioner who works without a manager or peer reviewer."],
      correctIndex: 2,
      explanation: "CI@MS is a many-to-many practice. The lone-genius practitioner validates 30 patterns and never publishes — <strong>the org pays the cost of every other practitioner re-discovering them</strong>. Contribution is the entry fee for the loop to compound."
    }
  ]
},
"data-types": {
  ...
  questions: [
    {
      prompt: "A practitioner pulls four weeks of actual P95 latency readings from Azure Monitor to baseline a process. What kind of data is this?",
    ...
    {
      prompt: "A team asserts CPU spikes are the root cause of latency breaches, citing a chart where CPU and latency both rise. Before standardizing a fix, what should a practitioner do?",
...
"dmaic": {
  ...
  questions: [
    ...
    },
    {
      ...
      correctIndex: 1,
      explanation: "Tollgates stop a costly engineering mistake: <strong>solving the wrong problem confidently</strong>. Each gate ensures evidence is sufficient before the next phase begins."
    },
    {
  ...
  questions: [
    ...
    },
    {
      ...
      correctIndex: 1,
      explanation: "Cpk &lt; 1.0 means the process is <strong>not capable</strong>. Hitting the spec is luck, not capability. The practitioner&rsquo;s job is to translate the number into &ldquo;trouble ahead&rdquo; before the next load increase causes a breach."
    },
    {
...
"p-value": {
  ...
  explanation: "<p><strong>What it is.</strong> A p-value answers one narrow question: <em>if there were truly no effect (the null hypothesis, H<sub>0</sub>, is true), how likely is it that random sampling alone would produce a result at least as extreme as what I observed?</em> A small p-value means the data would be surprising under 'no effect,' so you have evidence against the null. A common threshold (the significance level, &alpha;) is 0.05. <strong>What it is not:</strong> a p-value is <em>not</em> the probability that the null is true, not the probability your result happened by chance, and not a measure of how big or important the effect is. p = 0.03 does not mean '97% chance the improvement is real.'</p><p><strong>How to calculate it.</strong> The mechanics are always the same five steps: (1) State the null (e.g., 'the mean latency after the change equals the mean before') and the alternative. (2) Choose the test that matches your data and question&mdash;a <strong>two-sample t-test</strong> to compare two means, a <strong>two-proportion z-test</strong> or <strong>chi-square</strong> to compare defect/pass rates, <strong>ANOVA</strong> for more than two groups, or a non-parametric test (Mann-Whitney) when the data are skewed. (3) Compute the test statistic from the sample (for a t-test, roughly the difference in means divided by its standard error). (4) Find the p-value: the area in the tail(s) of that statistic's distribution beyond the value you observed&mdash;in practice you read it from software (Python's scipy.stats, R, Minitab, even a spreadsheet) rather than by hand. (5) Compare to &alpha;: if p &le; &alpha;, reject the null; if p &gt; &alpha;, you have insufficient evidence to reject it.</p><p><strong>Why it matters to CI.</strong> Continuous improvement is full of before/after comparisons, and every process has natural variation. Without a test, a team will see 'incidents dropped from 8 to 6' and declare a win&mdash;when 6 might be an ordinary fluctuation. The p-value is the <strong>convergent-thinking gate</strong>: it forces evidence before commitment and guards against premature convergence. In DMAIC <strong>Analyze</strong> it confirms that a suspected factor X is really associated with the outcome Y, not coincidence. In <strong>Improve</strong> it confirms that the pilot change actually moved the metric beyond what noise could explain. It is the formal version of 'go slow to go fast.'</p><p><strong>Where, when, and how to use it.</strong> Use it when you have variation and need to distinguish signal from noise, and when the data assumptions of your chosen test are reasonably met. Decide &alpha; <em>and</em> the sample size <em>before</em> collecting data (a power analysis tells you how many observations you need to detect an effect you would care about). Pair the p-value with a <strong>control chart</strong> (is the process even stable?) and always report it alongside an <strong>effect size and confidence interval</strong> so the audience sees both 'is it real?' and 'is it big enough to matter?'</p><p><strong>How it is misused.</strong> (1) <strong>p-hacking / fishing:</strong> slicing the data into many subgroups or testing many metrics until something lands under 0.05, then reporting only that. (2) <strong>Optional stopping:</strong> peeking at the running result and stopping the moment p dips below 0.05&mdash;this manufactures false positives. (3) <strong>Statistical vs practical significance:</strong> with a huge sample, a trivial 2&nbsp;ms latency change can be 'significant' yet meaningless; conversely a real improvement can miss 0.05 on a tiny sample. (4) <strong>Accepting the null:</strong> reading p &gt; 0.05 as 'proven no difference'&mdash;absence of evidence is not evidence of absence. (5) <strong>Misinterpretation:</strong> treating p as the probability the result is a fluke, or switching to a one-tailed test just to squeak under the threshold. (6) <strong>Ignoring assumptions:</strong> running a t-test on heavily skewed data or an unstable process.</p><p><strong>How to avoid misuse.</strong> Pre-register the hypothesis, the test, &alpha;, and the sample size before you look at the data. Define the minimum effect that would actually matter to the business, and report the <strong>effect size and confidence interval</strong>, not just the p-value. Size the sample with a power analysis and do not stop early. When you test many factors at once, <strong>correct for multiple comparisons</strong> (e.g., Bonferroni). Check the test's assumptions and use a non-parametric alternative when they fail. Phrase a high p-value as 'insufficient evidence of a difference,' never 'proof of no difference.' And replicate or confirm an important result before betting the roadmap on it.</p>",
  csamExample: "<p>A customer's ops lead proudly reports that a new triage process cut monthly incidents from 8 to 6&mdash;'a 25% improvement'&mdash;and wants to roll it out everywhere. The CSAM pumps the brakes: with only one month of data and historically high month-to-month variance, a two-sample test on the available history gives p = 0.18. That is not evidence of a real change; 6 sits comfortably within normal noise. The CSAM reframes it: 'This might be working, but we can't yet tell it apart from a good month. Let's keep the change, collect three more months, and watch it on a control chart before we commit the org to it.' This avoids premature convergence&mdash;declaring victory on chance&mdash;and protects the credibility of the next, evidence-backed recommendation.</p>",
  csaExample: "<p>A CSA pilots a caching change on an Azure Function and runs a two-sample t-test on response time: before &mu; = 850&nbsp;ms (n = 4,000), after &mu; = 320&nbsp;ms (n = 4,000), p &lt; 0.001. The improvement is real, not noise&mdash;and the CSA reports the effect size too: a 62% reduction with a 95% confidence interval of roughly 510&ndash;550&nbsp;ms, so it is both statistically and practically significant. On a second tweak, the test returns p = 0.04 but the mean drops only 3&nbsp;ms on a sample of 500,000 calls. The CSA explicitly does not claim a win: 'Statistically detectable, practically irrelevant.' By pre-deciding the sample size and the minimum meaningful effect, and refusing to p-hack across dozens of sub-metrics, the CSA keeps the Improve-phase evidence honest.</p>",
  recap: [
  ...
  questions: [
    {
      ...
      options: [
        ...
        "The probability that the improvement is real and worth deploying.",
        "The size of the effect the change produced."
      ],
      correctIndex: 1,
      explanation: "A p-value is the probability of data <strong>at least as extreme</strong> as observed <em>if the null were true</em>. It is not the probability the null is true, not the chance the result is a fluke, and not a measure of how big the effect is."
    },
    {
      prompt: "In a DMAIC project, where does the p-value most naturally belong?",
      options: [
        "In Define, to write the problem statement.",
        "In Analyze and Improve, to confirm a factor is really related to the outcome and that a change moved the metric beyond noise.",
        "In Control, as the only tool for ongoing monitoring.",
        "It has no role in CI; it is purely academic."
      ],
      correctIndex: 1,
      explanation: "The p-value is the <strong>evidence gate</strong> in Analyze (is X really associated with Y?) and Improve (did the change beat random variation?). It is the convergent-thinking step that stops a team declaring victory on noise."
    },
    {
      prompt: "A test on 500,000 requests shows a 3 ms latency reduction with p = 0.04. What is the right interpretation?",
      options: [
        "A clear win&mdash;p &lt; 0.05, so deploy it everywhere immediately.",
        "Statistically detectable but practically irrelevant; significance is not the same as importance.",
        "The test is invalid because the sample is too large.",
        "The p-value proves the 3 ms effect is exactly correct."
      ],
      correctIndex: 1,
      explanation: "With a very large sample, a trivial effect can clear the &alpha; threshold. This is the <strong>statistical-vs-practical significance</strong> trap: always report effect size, and judge the change against a minimum meaningful difference."
    },
    {
      prompt: "Which of these is a classic misuse of p-values?",
      options: [
        "Deciding alpha and the sample size before collecting data.",
        "Testing many subgroups or metrics until one lands under 0.05 and reporting only that one (p-hacking).",
        "Reporting a confidence interval alongside the p-value.",
        "Correcting for multiple comparisons when running many tests."
      ],
      correctIndex: 1,
      explanation: "<strong>p-hacking</strong> (and optional stopping&mdash;peeking and stopping when p dips below 0.05) manufactures false positives. The other options are exactly the guardrails that prevent misuse."
    },
    {
      prompt: "A comparison returns p = 0.18. What is the correct conclusion?",
      options: [
        "It proves there is no difference between the two groups.",
        "There is insufficient evidence to reject the null; you cannot yet distinguish the effect from noise.",
        "The improvement is 18% likely to be real.",
        "You should switch to a one-tailed test to get under 0.05."
      ],
      correctIndex: 1,
      explanation: "A high p-value means <strong>insufficient evidence of a difference</strong>&mdash;absence of evidence is not evidence of absence. It does not prove equality, and reaching for a one-tailed test to force significance is itself misuse."
    }
  ]
},
"value": {
  ...
  whatYouGain: [
    ...
    "The ability to justify practitioner hours to leadership through outcomes, not effort",
  ...
  recap: [
    "Value is customer-defined and outcome-shaped—measured in the customer's currency ($ saved, downtime avoided, time-to-market, secure-score points), never in practitioner activity",
    ...
    "Report value at EBR/QBR in one slide, customer's terms, no practitioner activity metrics—this slide is worth more for renewal than 30 slides of activity"
  ],
  questions: [
    {
      prompt: "Who defines value in a Lean engagement?",
      options: [
        "The practitioner, based on technical excellence.",
    ...
    {
      prompt: "A practitioner classifies the last 90 days as 38% VA, 22% NNVA, 40% NVA. What does this signal?",
      options: [
        ...
        "The practitioner should request more budget to absorb the overhead.",
    ...
    },
    {
      ...
      options: [
        ...
        "By total practitioner hours billed to the account.",
    ...
    {
      prompt: "A customer's CFO uses a Power BI spend dashboard the practitioner built to approve $1.2M in modernization. Is this dashboard VA, NVA, or NNVA?",
      options: [
      ...
      correctIndex: 1,
      explanation: "Value thinking is <strong>context-sensitive</strong>. A dashboard that leads to $1.2M of funded modernization isn't waste even though it's not directly transforming a service. The dashboard is the means; the funding is the value."
    }
  ]
},
"voc": {
  ...
  group: "Value & Quality Definition",
  duration: "30 min",
  executiveSummary: "Voice of the Customer (VOC) is the structured capture of what customers actually need, expressed in their own words, and the translation of those needs into measurable requirements (CTQs). Without VOC, teams optimize what is convenient to measure instead of what customers value—which wastes engineering effort and misdirects engagement priorities.",
  whatYouGain: [
...
"waste": {
  ...
  group: "Value & Quality Definition",
  duration: "30 min",
  executiveSummary: "The 8 Wastes (DOWNTIME—Defects, Overproduction, Waiting, Non-utilized talent, Transportation, Inventory, Motion, Extra-processing) are Lean's 360° checklist for spotting non-value-add activity in any system. For practitioners, the 8 Wastes translate cleanly to the Azure estate: over-provisioned RUs, idle VMs, stale environments, redundant observability stacks, unnecessary data egress, engineers stuck on toil, slow incident response loops, and redundant approval gates. Use the 8 Wastes as a structured walk-through during cost optimizations, reliability reviews, and modernization assessments—every category surfaces a different class of opportunity.",
  whatYouGain: [
  ...
  questions: [
    ...
    {
      prompt: "A practitioner was asked to find $200K/yr in savings. Rightsizing found $90K. A structured 8 Wastes walk found another $260K (idle subs, redundant observability, egress, toil). What's the lesson?",
      options: [
        "Rightsizing always misses most of the savings.",
        "Looking at only one waste (Overproduction) misses Inventory, Extra-processing, Transportation, and Non-utilized talent — a structured pass across all eight wastes surfaces savings a single-waste view misses.",
        "The practitioner should have done a WAF assessment instead.",
...
"sipoc": {
  ...
  questions: [
    ...
    },
    {
      ...
      options: [
        "The dollars saved per practitioner hour.",
  ...
  group: "Process Mapping & Analysis",
  duration: "30 min",
  executiveSummary: "A Pareto chart ranks categories in descending order by impact (incident count, downtime, $, RU/s consumed) and overlays a cumulative percentage line so the 'vital few' causes driving ~80% of impact are immediately visible. It is the prioritization tool practitioners use for EBR/QBR prep, WAF reviews, cost optimization, and escalation triage—weight by business impact, not raw frequency.",
  whatYouGain: [
    "Rank problems by business impact, not by count, so practitioner hours land on the vital few",
  ...
  questions: [
    {
      prompt: "A customer has 4,200 Azure Advisor alerts. How should you build the Pareto to prioritize practitioner effort?",
      options: [
    ...
    },
    {
      ...
      options: [
        ...
        "Measure total practitioner hours and compare to original estimate."
      ],
      correctIndex: 1,
  ...
  whatYouGain: [
    ...
    "Create a shared vocabulary across practitioner, customer, and PG conversations"
  ],
  explanation: "<p>The 6Ms—Man, Machine, Method, Material, Measurement, Mother Nature—are scaffolding for systematic cause exploration. In cloud/practitioner vocabulary: Man = people, skills, on-call rotation; Machine = Azure service, SDK, runtime, tier, SKU; Method = process, queries, partition-key strategy, deployment flow, rollout pattern; Material = configuration, secrets, IaC, image versions, container manifests, RU/s settings, NSGs, dependencies; Measurement = observability, alerting, SLOs, dashboards, sampling rates, retention; Mother Nature = traffic patterns, time-of-day, region peaks, marketing campaigns, upstream bursts, holidays, customer behavior shifts.</p><p>Use the 6Ms as a checklist during postmortems, WAF reviews, and reliability assessments. Walk every M, even if the answer is 'no contributor here, evidence X confirms.' The discipline matters more than the volume. After evidence, score each M as primary, contributing, or non-contributing. Direct effort to primaries; remediate contributing causes opportunistically; document non-contributing ones as ruled out. Some practitioners add Management as a 7th M for org/funding/strategy causes—for practitioner work this is often where 5 Whys terminates.</p><p>Translate vocabulary to the audience. 'Man' is fine internally but reads as gendered. Prefer 'People &amp; Skills.' Use whichever vocabulary the room will engage with.</p>",
...
"takt-time": {
  ...
  group: "Workplace, Flow & Standardization",
  duration: "30 min",
  executiveSummary: "Takt time is the rate at which a process must produce to meet customer demand: takt = available time / customer demand. If demand is 480 requests per 8-hour day, takt is 1 minute per request. Cycle time slower than takt = unmet demand and growing queues. Cycle time faster than takt = overproduction waste unless paced down. For practitioners, takt applies to deploy cadence, alert response, incident triage pace, request throughput, and AI agent invocation rate. Use takt to size capacity, set SLOs, design Standard Work, and detect overproduction.",
  whatYouGain: [
...
"one-piece-flow": {
  ...
  explanation: "<p><strong>Two ways to move work.</strong> <em>Batch processing</em> groups units and moves them through a process as a block&mdash;every unit gets step one, then every unit gets step two, and so on. <em>One-piece flow</em> (single-piece or continuous flow) moves one unit through the steps at a time, ideally with a batch size of one. The difference sounds small but it changes the economics of the whole process.</p><p><strong>Why batching feels efficient.</strong> Batches amortize fixed costs. If a step has a costly setup&mdash;a machine changeover, an environment spin-up, a context switch, an approval ceremony&mdash;doing it once for fifty units instead of fifty times looks like a clear win. That saving is real, and it is why batch sizes grow: every changeover or hand-off has a transaction cost, and large batches spread that cost thin.</p><p><strong>The hidden cost of batching.</strong> Large batches carry three penalties that don't show up in a per-step efficiency number. <strong>Lead time</strong> balloons: a unit finished early at step one still waits for the rest of the batch before it can move, so nothing completes until the whole batch is done. <strong>Work-in-progress</strong> piles up between steps&mdash;inventory that ties up cash, space, and attention. And <strong>defects hide</strong>: if step one introduces an error, you don't find out until the batch reaches a later inspection step, by which point the whole batch is affected. The bigger the batch, the later and more expensive the discovery.</p><p><strong>Why one-piece flow helps continuous improvement.</strong> Flow shortens the loop between cause and feedback. When units move one at a time, a defect at step one is caught at step two&mdash;one bad unit, not fifty. Problems become visible immediately instead of being buried inside a batch, which is the precondition for fixing them. WIP drops, so lead time drops with it (Little's Law: lead time = WIP / throughput). Shorter lead time means you learn faster, and learning faster is what continuous improvement is. Flow doesn't just deliver sooner; it exposes the problems that improvement work feeds on.</p><p><strong>Batch isn't always wrong.</strong> One-piece flow is the ideal, not a law. When changeover or transaction cost is genuinely high and can't be reduced, a larger batch may be the right economic choice&mdash;the classic trade-off between holding cost (which favors small batches) and setup cost (which favors large ones). The Lean move, though, is to attack the setup cost rather than accept big batches: <strong>SMED</strong> (single-minute exchange of die) reduces changeover time so smaller batches become affordable. Lower the cost of switching and the economic batch size shrinks toward one.</p><p><strong>How to move toward flow.</strong></p><ul><li><strong>Shrink the batch:</strong> cut batch size in deliberate steps and watch lead time and quality respond&mdash;don't wait for a perfect batch-size-of-one redesign.</li><li><strong>Reduce changeover (SMED):</strong> make setups faster and cheaper so small batches stop being expensive.</li><li><strong>Balance the steps:</strong> flow stalls at the slowest step; level cycle times so units don't pool in front of a bottleneck.</li><li><strong>Limit WIP:</strong> cap the work between steps (kanban) so the process pulls one unit at a time instead of pushing batches.</li><li><strong>Connect the steps:</strong> shorten the distance and delay between operations so a finished unit moves immediately to the next step.</li></ul>",
  csamExample: "<p>A CSAM's customer migrates workloads in big quarterly batches: fifty applications are assessed, then all fifty are remediated, then all fifty are cut over on one weekend. The batch approach feels efficient&mdash;tooling is set up once&mdash;but it hides risk. Nothing is actually 'done' for months, problems found during cutover apply to the whole batch at once, and a design flaw discovered late means re-touching all fifty. The CSAM reframes it as a flow problem: move applications one (or a few) at a time through assess &rarr; remediate &rarr; cut over, completing each before starting the next wave. Now the first app is live in weeks instead of a quarter, the lessons from app one improve apps two through fifty, and a bad pattern is caught on a single workload instead of fifty. The CSAM ties it back to CI: 'Small batches give us a feedback loop&mdash;every migration teaches the next one, and the customer sees value continuously instead of all at the end.'</p>",
  csaExample: "<p>A CSA is helping a team that ships changes in large, infrequent releases&mdash;dozens of merged changes bundled into one big-bang deployment every few weeks. It's batch processing: setup (the release) is expensive, so they do it rarely and stuff each one full. The cost is hidden in lead time and risk&mdash;a change written on day one waits weeks to ship, and when the release breaks, dozens of changes are suspect at once, making root cause slow. The CSA moves the team toward one-piece flow with CI/CD: each change flows through build, test, and deploy on its own, behind feature flags, so a single change reaches production in hours. Defects are isolated to one change instead of a batch, time-to-detect drops, and rollback is simple. To make small batches affordable, the CSA invests in SMED-style work&mdash;automating the deployment pipeline so the 'changeover cost' of a release falls toward zero. The control chart on deployment lead time tightens, and the team improves on a stable, fast loop.</p>",
  recap: [
  ...
  questions: [
    {
      prompt: "What is the defining difference between batch processing and one-piece flow?",
      options: [
        "One-piece flow uses larger batches to be more efficient.",
        "Batch processing moves a group of units through each step as a block, while one-piece flow moves a single unit through the steps at a time.",
        "Batch processing has no setup cost, while one-piece flow does.",
        "One-piece flow only applies to manufacturing, not knowledge work."
      ],
      correctIndex: 1,
      explanation: "<strong>Batch processing</strong> completes a step for an entire group before the group moves on; <strong>one-piece flow</strong> moves a single unit through the steps&mdash;ideally a batch size of one&mdash;so units don't wait for the rest of a batch."
    },
    {
      prompt: "A step introduces a defect at the start of a 50-unit batch, but inspection happens only at the final step. What is the consequence of the large batch?",
      options: [
        "The defect is caught immediately on the first unit.",
        "All 50 units are affected before the defect is discovered, making detection late and rework expensive.",
        "The defect disappears because batching averages out errors.",
        "Batch size has no effect on when defects are found."
      ],
      correctIndex: 1,
      explanation: "In a batch, a defect introduced early isn't discovered until the batch reaches a later inspection step&mdash;by then the <strong>whole batch is affected</strong>. Smaller batches (toward one-piece flow) catch the problem on one unit, not fifty."
    },
    {
      prompt: "Why does one-piece flow accelerate continuous improvement?",
      options: [
        "It eliminates the need to measure the process.",
        "It shortens the feedback loop—defects surface immediately on a single unit and problems become visible instead of being buried in a batch.",
        "It guarantees defects never occur.",
        "It increases work-in-progress so there is more to improve."
      ],
      correctIndex: 1,
      explanation: "Flow tightens the loop between cause and feedback: a problem shows up on one unit at the next step, WIP and lead time fall (Little's Law), and abnormalities become visible&mdash;the preconditions improvement work depends on."
    },
    {
      prompt: "Setup/changeover cost for a step is genuinely high. What is the Lean response, rather than simply accepting large batches?",
      options: [
        "Increase the batch size indefinitely to amortize setup.",
        "Reduce the changeover cost itself (e.g., SMED) so smaller batches become economical.",
        "Stop measuring lead time.",
        "Eliminate all inspection steps."
      ],
      correctIndex: 1,
      explanation: "The trade-off between setup cost (favors large batches) and holding cost (favors small) sets the economic batch size. The Lean move is to <strong>attack the setup cost with SMED</strong> so the economic batch size shrinks toward one."
    },
    {
      prompt: "According to Little's Law, what happens to lead time as one-piece flow reduces work-in-progress at a steady throughput?",
      options: [
        "Lead time increases proportionally.",
        "Lead time decreases proportionally—less WIP at the same throughput means shorter lead time.",
        "Lead time is unaffected by WIP.",
        "Throughput must double for lead time to change."
      ],
      correctIndex: 1,
      explanation: "Little's Law: lead time = WIP / throughput. Holding throughput steady, <strong>cutting WIP cuts lead time proportionally</strong>&mdash;which is exactly what moving from large batches to one-piece flow does."
    }
  ]
},
...
"supermarket": {
  ...
  explanation: "<p><strong>What a supermarket is.</strong> A supermarket is a controlled, deliberately sized inventory store sitting between an upstream (supplying) process and a downstream (consuming) process. Taiichi Ohno borrowed the idea from American grocery stores: shoppers take what they need from the shelf, and staff restock only what was removed. In a Lean supermarket the <em>downstream</em> process is the customer&mdash;it withdraws what it needs, and the empty space it leaves becomes a <strong>kanban</strong> signal telling the <em>upstream</em> process to make exactly enough to replenish it. Production is pulled by real consumption, not pushed by a forecast.</p><p><strong>Why it exists: pull without continuous flow.</strong> The Lean ideal is one-piece flow&mdash;units moving one at a time with no inventory between steps. But sometimes you can't connect two steps into continuous flow: they run at very different cycle times, sit far apart, share a resource, or the upstream step must run in batches (a long changeover, a shared machine, a slow build). A supermarket is the next-best option. It lets the two processes run decoupled at their own natural pace while a capped buffer absorbs the difference&mdash;and crucially, the cap and the replenishment signal stop the upstream from overproducing.</p><p><strong>How it avoids batching and overproduction.</strong> An uncontrolled pile of WIP invites the worst Lean waste&mdash;overproduction&mdash;because the upstream just keeps pushing. A supermarket is different: it has a ceiling. When the shelf is full, the kanban signals stop, so the upstream stops. The upstream replenishes in small amounts matched to what was actually withdrawn, which keeps batches small and bounded instead of large and speculative. The supermarket converts 'make as much as you can' into 'replace only what the customer took.'</p><p><strong>Supermarket vs FIFO lane vs flow.</strong> Three options for a hand-off, in order of preference: <strong>(1) Continuous flow</strong>&mdash;connect the steps so a unit passes straight through; use this whenever you can. <strong>(2) FIFO lane</strong>&mdash;a sequenced, capped line where items flow first-in-first-out; use it when you can't fully connect the steps but the downstream consumes in the same order the upstream produces. <strong>(3) Supermarket</strong>&mdash;a stocked store the downstream picks from; use it when the downstream withdraws unpredictably or selects among several item types, so a strict sequence won't work. Reach for the simplest option that fits; a supermarket is a deliberate compromise, not the goal.</p><p><strong>Where supermarkets are a best practice.</strong></p><ul><li>Between processes with <strong>different cadences</strong> or cycle times that can't be balanced into direct flow.</li><li>After an upstream step that <strong>must batch</strong>&mdash;a long changeover, a shared or monopoly resource, a slow build or bake step.</li><li>When the downstream <strong>withdraws a varying mix</strong> of standard, repeatable items and needs them available on demand.</li><li>To <strong>decouple an unreliable or distant</strong> upstream while still capping inventory and keeping a clear pull signal.</li></ul><p><strong>Where to avoid them.</strong></p><ul><li>When <strong>continuous flow is achievable</strong>&mdash;a supermarket adds inventory, cost, and space, and it hides problems behind a buffer. Don't institutionalize a store you could eliminate.</li><li>For <strong>one-off, custom, or rarely demanded items</strong>&mdash;you can't replenish a stock of things that are never reordered; use make-to-order or a FIFO lane.</li><li>For <strong>perishable or fast-changing items</strong> (or expensive-to-hold ones), where standing stock goes stale or costly before it's pulled.</li><li>When the supermarket becomes a <strong>crutch</strong> that masks upstream unreliability, long changeovers, or quality problems you should be fixing&mdash;the buffer should shrink over time, not grow.</li></ul><p><strong>The CI mindset.</strong> A supermarket is controlled inventory with a ceiling: better than an uncontrolled batch, worse than true flow. Use it where flow isn't yet possible, size it as small as the process allows, and treat its size as a metric to drive down. Every reduction in the supermarket exposes the next constraint to fix&mdash;moving the system one step closer to one-piece flow.</p>",
  csamExample: "<p>A CSAM's customer needs project environments on demand, but provisioning a fresh one takes hours, so teams either wait (delay) or someone spins up a big batch of environments in advance that mostly sit idle (overproduction and cost). The CSAM introduces a supermarket: a small, capped pool of pre-provisioned standard environments. When a team withdraws one, that withdrawal triggers automation to provision a single replacement&mdash;pull, not a speculative batch. The pool decouples slow provisioning from fast, bursty demand while a hard cap keeps idle cost bounded. The CSAM is explicit about the limits: for a bespoke, one-off environment with special requirements there's nothing to restock, so that stays make-to-order; and as provisioning automation gets faster, the pool should shrink toward zero. 'The pre-provisioned pool is a bridge,' the CSAM explains, 'capped today because provisioning is slow&mdash;our job is to make it fast enough that we need fewer on the shelf.'</p>",
  csaExample: "<p>A CSA is designing a deployment platform where building a hardened golden image is slow, but deployments need images immediately. Building on demand stalls deployments; pre-building every possible image in bulk wastes storage and ships stale artifacts. The CSA sets up a supermarket of pre-baked base images for the handful of standard, frequently used configurations: when a deployment consumes one, a pipeline rebuilds just that image to replenish the shelf&mdash;a withdrawal-triggered pull that keeps batches small and the stock bounded. The same pattern backs a warm pool of pre-pulled container nodes that decouples slow cold starts from fast scale-up demand. The CSA flags where not to use it: rarely used or rapidly changing images shouldn't be stocked&mdash;they go stale&mdash;so those are built to order; and where autoscaling can genuinely keep up from zero, a standing warm pool is just cost. The control target is the pool size: shrink it as build and start-up times improve, moving the platform toward flow.</p>",
  recap: [
  ...
  questions: [
    {
      prompt: "What is a Lean supermarket?",
      options: [
        "An uncontrolled pile of work-in-progress between two steps.",
        "A deliberately sized, capped store of inventory from which a downstream process withdraws, signaling the upstream to replenish exactly what was taken.",
        "A forecast that tells the upstream process how much to push downstream.",
        "A storage area where finished goods are held until a quarterly batch ships."
      ],
      correctIndex: 1,
      explanation: "A supermarket is a <strong>controlled, capped store</strong> between processes. The downstream withdraws what it needs and the empty space becomes a kanban signal telling the upstream to replenish just that amount&mdash;pull, not push."
    },
    {
      prompt: "Why would you use a supermarket instead of connecting two steps into continuous one-piece flow?",
      options: [
        "Because inventory is always preferable to flow.",
        "Because the steps can't be linked into continuous flow—different cycle times, distance, a shared resource, or an upstream step that must batch.",
        "Because supermarkets eliminate the need for a pull signal.",
        "Because it lets the upstream produce as much as it wants."
      ],
      correctIndex: 1,
      explanation: "One-piece flow is the ideal, but when steps run at different cadences, sit far apart, share a resource, or must batch, you can't connect them directly. A supermarket is the <strong>next-best control</strong>: it decouples them while capping inventory and keeping a pull signal."
    },
    {
      prompt: "How does a supermarket help avoid batching and overproduction?",
      options: [
        "It removes all limits so the upstream can build ahead.",
        "It has a ceiling—when the shelf is full the kanban signals stop, so the upstream replenishes only what was withdrawn, in small bounded amounts.",
        "It requires the downstream to take the entire stock at once.",
        "It replaces pull signals with a monthly production forecast."
      ],
      correctIndex: 1,
      explanation: "Unlike an uncontrolled WIP pile, a supermarket is <strong>capped</strong>. A full shelf stops the replenishment signal, so the upstream stops; it replaces only what was consumed, keeping batches small instead of large and speculative."
    },
    {
      prompt: "A hand-off can't be fully connected into continuous flow, but the downstream consumes items in the same order the upstream produces them. Which option is preferred over a supermarket?",
      options: [
        "A larger supermarket with more item types.",
        "A FIFO lane—a sequenced, capped line that preserves first-in-first-out order.",
        "An uncapped buffer.",
        "A monthly batch release."
      ],
      correctIndex: 1,
      explanation: "When sequence is preserved, a <strong>FIFO lane</strong> is simpler than a supermarket: a capped, first-in-first-out line. Reach for the simplest option that fits&mdash;flow, then FIFO, then a supermarket for when the downstream withdraws unpredictably or picks among item types."
    },
    {
      prompt: "Which situation is a poor fit for a supermarket?",
      options: [
        "Two standard, repeatable processes running at different cadences.",
        "One-off, custom items that are never reordered, or fast-changing items that go stale before they're pulled.",
        "An upstream step with a long changeover that must run in batches.",
        "A downstream process that withdraws a varying mix of standard items on demand."
      ],
      correctIndex: 1,
      explanation: "You can't replenish a stock of items that are never reordered, and perishable or fast-changing items go stale on the shelf. Those call for <strong>make-to-order or a FIFO lane</strong>. Also avoid supermarkets when true continuous flow is achievable or when the buffer just masks a problem to fix."
    }
  ]
},
...
"pull-vs-push": {
  ...
  explanation: "<p><strong>The core difference: what starts the work.</strong> Push and pull answer one question differently&mdash;<em>when does a step begin working?</em> In a <strong>push</strong> system, a step starts because a schedule or forecast told it to: produce this much by this date and hand it to the next step, ready or not. In a <strong>pull</strong> system, a step starts only when the <em>downstream</em> step signals that it has consumed something and needs a replacement. Work is drawn through the process by real demand instead of being shoved through by a plan.</p><p><strong>Why push causes trouble.</strong> Push is driven by a forecast, and forecasts are never exactly right. When upstream steps produce to plan regardless of what downstream can absorb, the mismatch turns into <strong>inventory</strong>: work-in-progress piling up between steps. That inventory is the worst Lean waste&mdash;<strong>overproduction</strong>&mdash;and it brings its own costs: longer lead time (a unit waits behind everything already queued), tied-up cash and space, and hidden defects (a problem introduced upstream sits undetected inside the pile until someone finally works it). Push keeps every step busy locally while the system as a whole slows down and goes blind to its own problems.</p><p><strong>Why pull helps.</strong> A pull system caps the amount of work in the system to what downstream actually wants. Because nothing starts without a downstream signal, WIP stays low&mdash;and by Little's Law (lead time = WIP / throughput), low WIP means short lead time. Pull also makes problems <em>visible</em>: when a downstream step stops pulling, the whole line stops, so a bottleneck or a defect surfaces immediately instead of being buried under inventory. That visibility plus the short feedback loop is exactly what continuous improvement needs&mdash;you see the constraint, you fix it, and you see the result quickly.</p><p><strong>How pull works in practice.</strong> Pull is implemented with explicit signals: <ul><li><strong>Kanban</strong>&mdash;a card or signal that authorizes the upstream step to produce or move one unit; no card, no work.</li><li><strong>Supermarkets</strong>&mdash;a capped store the downstream withdraws from; the gap left behind is the replenishment signal.</li><li><strong>WIP limits</strong>&mdash;a hard cap on how much work can sit in a stage; a full stage blocks upstream from starting more.</li></ul>All three enforce the same rule: upstream may only act in response to real downstream consumption.</p><p><strong>Pull is not the same as one-piece flow.</strong> Continuous one-piece flow is the ideal&mdash;units move straight through with no inventory at all. Pull is how you run a process when you can't achieve pure flow: it still allows small, capped buffers (supermarkets), but it controls them with demand signals so they never grow into uncontrolled push inventory. Flow first; where you can't flow, pull.</p><p><strong>Where a measured amount of push still fits.</strong> Pull is the default, but it isn't absolute. Push (forecast-driven) can be the right call when: the total lead time to make something is longer than the customer is willing to wait, so you must start before the order exists; demand is highly seasonal and you build ahead to level capacity; a one-off or first-of-its-kind item has no repeat demand to pull against; or you are pre-positioning capacity (not finished work) against a known event. The mature pattern is usually a hybrid&mdash;push to a strategic decoupling point, then pull from there to the customer&mdash;sizing any forecast-driven buffer deliberately and shrinking it as lead times improve.</p>",
  csamExample: "<p>A CSAM's customer runs their modernization program on a push model: a yearly plan assigns dozens of workloads to be migrated on a fixed schedule, and teams start each one because the plan says so&mdash;regardless of whether the receiving platform, security reviews, or downstream owners are ready. The result is a pile of half-finished migrations stuck waiting on dependencies: lots of motion, little completion, and problems discovered late. The CSAM reframes it as push vs pull. Instead of releasing work by the calendar, the team pulls: a new migration starts only when the platform team has capacity to accept it and the previous one is truly done, with a WIP limit capping how many can be in flight. Now work is drawn by real readiness, finished migrations actually land, and stalls become visible immediately instead of hiding in a backlog. The CSAM ties it to CI: 'Pull caps what we start to what we can finish&mdash;so we see the real bottleneck and improve it, instead of starting more and hoping.'</p>",
  csaExample: "<p>A CSA is helping a data team whose pipeline runs on push: every source system dumps its full extract on a fixed schedule into the processing tier, whether or not the processors are caught up. On heavy days the processing tier is buried under a backlog (overproduction, long latency, paged engineers); the pile hides which stage is actually the constraint. The CSA converts it to pull: producers write to a queue, and consumers pull work only as they finish the previous item, with a WIP limit so the system can't be flooded and autoscaling sized to the steady pull rate rather than the raw dump. Backlogs stop forming, end-to-end latency drops as WIP falls (Little's Law), and the bottleneck stage becomes obvious on the dashboard. The CSA notes the exception: for a genuinely long-lead, predictable nightly aggregation the team still pre-computes ahead of demand&mdash;a deliberate, bounded push&mdash;but everything downstream of that point is pulled.</p>",
  recap: [
  ...
  questions: [
    {
      prompt: "What is the fundamental difference between a push and a pull system?",
      options: [
        "Push uses smaller batches than pull.",
        "In push, work starts from a forecast or schedule; in pull, work starts only when a downstream step signals real demand.",
        "Pull requires more inventory than push.",
        "Push has no defects, while pull does."
      ],
      correctIndex: 1,
      explanation: "The distinction is about <strong>what triggers work to start</strong>. Push releases work to a plan/forecast and sends it downstream regardless of readiness; pull starts work only in response to a downstream demand signal."
    },
    {
      prompt: "Why does a push system tend to create waste?",
      options: [
        "It never keeps the upstream steps busy.",
        "Producing to a forecast regardless of downstream readiness causes overproduction—WIP piles up, lead time grows, and defects hide inside the inventory.",
        "It caps work-in-progress too aggressively.",
        "It makes problems too visible to ignore."
      ],
      correctIndex: 1,
      explanation: "Push is forecast-driven, and the mismatch with real demand turns into <strong>overproduction</strong>&mdash;the worst Lean waste. The resulting WIP lengthens lead time, ties up cash and space, and hides defects until much later."
    },
    {
      prompt: "How does a pull system shorten lead time?",
      options: [
        "By increasing throughput regardless of WIP.",
        "By capping WIP to real demand—lower WIP at a given throughput means shorter lead time (Little's Law).",
        "By removing the customer from the process.",
        "By producing ahead of demand to build a buffer."
      ],
      correctIndex: 1,
      explanation: "Pull caps the work in the system to what downstream pulls, keeping WIP low. By Little's Law (lead time = WIP / throughput), lower WIP at the same throughput <strong>shortens lead time</strong>."
    },
    {
      prompt: "Which of these is a mechanism used to implement a pull system?",
      options: [
        "A quarterly production forecast pushed to every step.",
        "Kanban signals, supermarkets, and WIP limits—where upstream acts only on real downstream consumption.",
        "Releasing all work at the start of the period.",
        "Removing every buffer so steps never coordinate."
      ],
      correctIndex: 1,
      explanation: "Pull is enforced with explicit signals: <strong>kanban</strong> cards that authorize one unit of work, <strong>supermarkets</strong> whose gaps signal replenishment, and <strong>WIP limits</strong> that block upstream when a stage is full."
    },
    {
      prompt: "When is a measured amount of push still appropriate?",
      options: [
        "Whenever you want to keep every step locally busy.",
        "When total lead time exceeds the wait the customer will tolerate, for seasonal build-ahead, or one-off items—often as a hybrid that pushes to a decoupling point and pulls from there.",
        "Always, because push is simpler than pull.",
        "Never—push is always wrong under any condition."
      ],
      correctIndex: 1,
      explanation: "Pull is the default, not an absolute. Forecast-driven <strong>push</strong> fits when you must start before an order exists (long lead time), to level seasonal demand, or for one-off items&mdash;commonly a hybrid: push to a strategic decoupling point, then pull to the customer."
    }
  ]
},
