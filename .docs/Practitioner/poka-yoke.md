...
- Apply engineering poka-yokes: a policy engine (deny mode), role-based access control (least privilege), schema validation, CI gates, resource locks, workload identity

## The Concept, Explained

**Poka-Yoke has two classes:**

- **Prevention:** Error is physically or logically impossible. A policy engine denies the action. The resource cannot be deleted. The tag cannot be non-immutable.
...
**Common poka-yokes in engineering:** a policy engine (deny/audit modes), role-based access control scoped least-privilege, schema validation, pre-commit hooks (secret scanning, lint), pipeline gates, resource locks (cannot-delete/read-only), workload identity, immutable infrastructure, type systems and lints.
...
A CI practitioner stops recurring human-error outages not with another 'be more careful' reminder but with poka-yoke — designing the mistake out. A policy engine denies the invalid configuration outright, a required field can't be left wrong, and the pipeline refuses a deploy that skips a gate. The error rate for that class of mistake drops to zero because the system no longer allows it. Mistake-proofing is how the practitioner turns reliability from a matter of individual vigilance into a property of the process itself.
...
- Common poka-yokes: a policy engine (deny mode), role-based access control (least privilege), schema validation, pre-commit hooks, CI gates, resource locks, workload identity.
...
Prevention makes the error **impossible** (a policy engine denies the action); detection catches it the moment it happens (schema validation rejects malformed config). Prevention is preferred when feasible.
...
- **B.** Apply a mechanism: a resource lock (cannot-delete), a policy-engine deny, and block non-interactive force flags in CI runners.
- **C.** Document the issue in a wiki and move on.
- **D.** Issue a strongly worded memo from leadership.

**Correct answer:** B. Apply a mechanism: a resource lock (cannot-delete), a policy-engine deny, and block non-interactive force flags in CI runners.
...
- **A.** Using a policy engine in deny mode for prod resources.
- **B.** Treating “training plus a checklist” as a poka-yoke when only a mechanism qualifies.
- **C.** Locking down role-based access control to least privilege.
- **D.** Using workload identity instead of secrets in code.
