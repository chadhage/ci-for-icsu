...
Engineering surfaces that count as gemba: the repo and IDE during a development task, the build and deploy as it runs, the ChatOps channel during an incident, the standup as it happens, on-call observations, usage telemetry. What gemba is *not*: a dashboard review, a status meeting, a slide-summarized walkthrough, or "executive gemba"—a 30-minute visit with a 6-person entourage where nobody works naturally.
